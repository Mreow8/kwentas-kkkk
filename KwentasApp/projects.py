        ...
        for key, value in result.val().items():
            ...
            if entry.get('year') is not None:
                ...
                if int(entry['year']) < current_year:
                    continuing_projects.append(entry)
                else:
                    current_projects.append(entry)

    # Sort entries by 'created_at' in descending order
    continuing_projects.sort(key=lambda x: x['created_at'], reverse=True)
    current_projects.sort(key=lambda x: x['created_at'], reverse=True)
    all_entries.sort(key=lambda x: x['created_at'], reverse=True)

    # Cache the data for future use (e.g., 1 hour)
    cache.set('project_entries', (continuing_projects, current_projects, all_entries), 3600)

    return continuing_projects, current_projects, all_entries
...
def continuing_projects(request):
    ...
    continuing_projects, _, all_entries = get_project_entries()
    ...
    if sort_option == 'oldest':
        continuing_projects.sort(key=lambda x: x['created_at'])  # Sort by oldest
    elif sort_option == 'recent':
        continuing_projects.sort(key=lambda x: x['created_at'], reverse=True)  # Sort by recent
    
    # Paginate the entries (sorted or unsorted)
    paginator = Paginator(continuing_projects, 10)  # Show 10 projects per page
...
def current_projects(request):
    ...
    continuing_projects, current_projects, all_entries = get_project_entries()
    ...
    # Apply sorting if an option is selected
    if sort_option == 'oldest':
        current_projects.sort(key=lambda x: x['created_at'])  # Sort by oldest
    elif sort_option == 'recent':
        current_projects.sort(key=lambda x: x['created_at'], reverse=True)  # Sort by recent
    
    # Paginate the entries (sorted or unsorted)
    paginator = Paginator(current_projects, 10)  # Show 10 projects per page
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
...
def search_projects(request, project_type):
    ...
    if project_type == 'continuing':
        ...
        year_condition = lambda year: year is not None and int(year) < datetime.now().year
